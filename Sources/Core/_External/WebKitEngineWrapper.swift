...
@MainActor
public final class WebKitEngineWrapper: NSObject, @preconcurrency WebEngineContract {
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if #available(macOS 10.15, *) {
            Task { @MainActor in
                ...
                case #keyPath(WKWebView.title):
                    if let url = webView.url, let title = webView.title, !title.isEmpty {
                        let webPage = WebPage(title: title, url: url, date: Date())
            ...
            DispatchQueue.main.async {
                switch keyPath {
                case #keyPath(WKWebView.url), #keyPath(WKWebView.canGoBack), #keyPath(WKWebView.canGoForward):
                    self.delegate?.didUpdateNavigationButtons(canGoBack: self.webView.canGoBack, canGoForward: self.webView.canGoForward)
                case #keyPath(WKWebView.estimatedProgress):
                    self.delegate?.didUpdateLoadingProgress(self.webView.estimatedProgress)
                case #keyPath(WKWebView.title):
                    if let url = self.webView.url, let title = self.webView.title, !title.isEmpty {
                        let webPage = WebPage(title: title, url: url, date: Date())
